...
export default function MobileBottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[200] bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-[0_-5px_25px_rgba(0,0,0,0.08)] safe-bottom">
      <div className="flex items-center justify-between h-16 px-1.5 gap-1">
        {/* 1. Ana Sayfa */}
        <Link
          href="/"
          className={`flex-1 h-13 flex flex-col items-center justify-center rounded-xl transition-all gap-1 ${
            isHomeActive
              ? "text-primary font-black bg-pink-50/80"
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">Ana Sayfa</span>
        </Link>
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">Hizmetler</span>
        </Link>

        {/* 3. Randevu Al (WhatsApp Priority Button) */}
        <a
          href={`${siteConfig.whatsappUrl}?text=${encodeURIComponent(siteConfig.whatsappDefaultMessage)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-[1.25] h-12 bg-[#25D366] hover:bg-[#1ebe5d] active:scale-95 text-white font-black text-[11px] uppercase tracking-wider rounded-xl shadow-md shadow-[#25D366]/25 flex items-center justify-center gap-1.5 transition-all"
        ...
        <a
          ...
          className="flex-1 h-13 flex flex-col items-center justify-center rounded-xl text-gray-700 hover:text-primary active:scale-95 transition-all gap-1"
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">Hemen Ara</span>
        </a>
      </div>
    </div>
  );
